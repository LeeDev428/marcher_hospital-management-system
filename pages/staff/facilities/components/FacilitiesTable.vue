...
const filteredRooms = computed(() => {
	...
	return roomStore.rooms.filter((r: any) => {
		const haystack = [
			r.identifier,
			r.building?.name,
			r.type,
...
watch([sortedRooms, pageSize], () => { currentPage.value = 1 })
...
<template>
	<!-- Loading State -->
	<div v-if="roomStore.loading" class="flex flex-col gap-2">
	...
	<div v-else class="overflow-x-auto">
		...
		<!-- Table -->
		<Table v-if="totalItems > 0" class="min-w-[900px]">
			<TableHeader>
				<TableRow>
			...
			<TableBody>
				...
				>
					<TableCell>{{ room.identifier }}</TableCell>
					...
					<TableCell>
						<span 
							...
							:class="{
								...
								'bg-gray-100 text-gray-800': !['available', 'occupied', 'maintenance'].includes(room.status)
							}"
						>
							{{ room.status }}
						</span>
					</TableCell>
					<TableCell :title="new Date(room.createdAt).toISOString()">
		...
		<div v-if="totalItems === 0" class="flex flex-col gap-2 justify-center items-center h-full py-8">
			<p class="text-sm text-gray-500">
				{{ debouncedQuery ? 'No rooms found matching your search.' : 'No Rooms Found' }}
			</p>
			<button 
				v-if="debouncedQuery"
				type="button"
