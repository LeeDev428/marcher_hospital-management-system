...
import { Client } from 'file://D:/Programming/Systems/Web-Systems/Nuxtjs-Nodejs-Expressjs/marcher_hospital-management-system/node_modules/minio/dist/esm/minio.mjs';
...
const _lazy_TSkQws = () => Promise.resolve().then(function () { return _fileName__get$1; });
const _lazy_6zw7Bh = () => Promise.resolve().then(function () { return index_post$1; });
...
const handlers = [
  ...
  { route: '/api/upload/:fileName', handler: _lazy_TSkQws, lazy: true, middleware: false, method: "get" },
  { route: '/api/upload', handler: _lazy_6zw7Bh, lazy: true, middleware: false, method: "post" },
...
const signAccessToken = (payload, subject) => {
...
const signRefreshToken = (payload, subject) => {
  ...
  try {
    ...
    if (user) {
      ...
      setCookie(ctx.event, "refreshToken", signRefreshToken(user, user.email), {
        ...
        expires: input.remember ? new Date(Date.now() + 1e3 * 60 * 60 * 24 * 30) : new Date(Date.now() + 1e3 * 60 * 60 * 24 * 7)
      });
...
const logout = protectedProcedure.mutation(async ({ ctx }) => {
  try {
...
patientAddressSchema.extend({
  id: z.string().uuid("Invalid patient address ID.")
});
const createPatientAddressSchema = patientAddressSchema;
z.object({
  id: z.string().uuid("Invalid patient address ID.")
});
...
contactTypeSchema.options.map((option) => ({
...
const createPatientContactSchema = patientContactSchema.omit({
  patientProfileId: true
});
patientContactSchema.partial().omit({
  patientProfileId: true
});

const patientEmploymentSchema = z.object({
  patientProfileId: z.string().uuid("Invalid patient profile ID."),
...
const createPatientEmploymentSchema = patientEmploymentSchema.omit({
  patientProfileId: true
});
patientEmploymentSchema.partial().omit({
  patientProfileId: true
});
...
relationshipSchema.options.map((option) => ({
  label: option.charAt(0) + option.slice(1).toLowerCase(),
  value: option
}));
...
const createPatientEmergencyContactSchema = patientEmergencyContactSchema.omit({
  patientProfileId: true
});
patientEmergencyContactSchema.partial().omit({
  patientProfileId: true
});
...
const createPatientConsentSchema = patientConsentSchema.omit({
  patientProfileId: true
});
patientConsentSchema.partial().omit({
  patientProfileId: true
});
...
maritalStatusSchema.options.map((option) => ({
  label: option.charAt(0) + option.slice(1).toLowerCase(),
  value: option
}));
bloodTypeSchema.options.map((option) => ({
...
const tablePatientProfileSchema = patientProfileSchema.extend({
  id: z.string().uuid("Invalid patient profile ID."),
  archived: z.boolean(),
...
const updatePatientProfileSchema = patientProfileSchema.partial().extend({
  id: z.string().uuid("Invalid patient profile ID."),
  addresses: z.array(createPatientAddressSchema).optional().nullable(),
  contacts: z.array(createPatientContactSchema).optional().nullable(),
  employments: z.array(createPatientEmploymentSchema).optional().nullable(),
...
const getPatientProfile = protectedProcedure.input(getPatientProfileSchema).query(async ({ ctx, input }) => {
...
const getActivePatientProfiles = protectedProcedure.input(querySchema).query(async ({ ctx, input }) => {
...
const getArchivedPatientProfiles = protectedProcedure.input(querySchema).query(async ({ ctx, input }) => {
  const { search, page, limit } = input;
  const { instancePrisma } = ctx;
  try {
    const patientProfiles = await instancePrisma.patientProfile.findMany({
...
const createPatientProfile = protectedProcedure.input(createPatientProfileSchema).mutation(async ({ ctx, input }) => {
  var _a, _b, _c, _d, _e;
  const { instancePrisma } = ctx;
  ...
  try {
    const patientProfile = await instancePrisma.patientProfile.create({
      data: {
        ...profile,
        addresses: {
          create: (_a = input.addresses) != null ? _a : void 0
        },
        contacts: {
          create: (_b = input.contacts) != null ? _b : void 0
        },
        employments: {
          create: (_c = input.employments) != null ? _c : void 0
        },
        emergencyContacts: {
          create: (_d = input.emergencyContacts) != null ? _d : void 0
        },
        consent: {
          create: (_e = input.consent) != null ? _e : void 0
        }
      }
    });
    return {
...
const updatePatientProfile = protectedProcedure.input(updatePatientProfileSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  ...
  try {
    const patientProfile = await instancePrisma.patientProfile.update({
      ...
      data: {
        ...
        addresses: {
          deleteMany: {},
          create: addresses != null ? addresses : void 0
        },
        contacts: {
          deleteMany: {},
          create: contacts != null ? contacts : void 0
        },
        employments: {
          deleteMany: {},
          create: employments != null ? employments : void 0
        },
        emergencyContacts: {
          deleteMany: {},
          create: emergencyContacts != null ? emergencyContacts : void 0
        },
        consent: {
          upsert: consent ? {
            create: consent,
...
const archivePatientProfile = protectedProcedure.input(getPatientProfileSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const unarchivePatientProfile = protectedProcedure.input(getPatientProfileSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
    await instancePrisma.patientProfile.update({
...
const patientProfilesRouter = createTRPCRouter({
  getPatientProfile,
...
appointmentStatusSchema.options.map((status) => ({
...
const appointmentSchema = z.object({
  ...
  // Optional for patients initially
  date: dateString,
  time: timeString,
...
const updatePatientAppointmentSchema = z.object({
  id: z.string().uuid("Invalid appointment ID."),
...
const isWithinOfficeHours = (time) => {
...
async function getCurrentPatientOrNull(instancePrisma, user) {
  if (!user) return null;
...
function pickName(user) {
  var _a, _b, _c, _d;
  const first = (user == null ? void 0 : user.firstName) || (user == null ? void 0 : user.given_name) || ((_b = (_a = user == null ? void 0 : user.name) == null ? void 0 : _a.split(" ")) == null ? void 0 : _b[0]) || "Unknown";
  const last = (user == null ? void 0 : user.lastName) || (user == null ? void 0 : user.family_name) || ((_d = (_c = user == null ? void 0 : user.name) == null ? void 0 : _c.split(" ")) == null ? void 0 : _d.slice(1).join(" ")) || "User";
  return { firstName: String(first).trim() || "Unknown", lastName: String(last).trim() || "User" };
}
async function ensurePatientProfile(prisma, user) {
...
const getAppointments = protectedProcedure.input(getAppointmentsSchema).query(async ({ ctx, input }) => {
  ...
  try {
    ...
    const staffProfiles = await globalPrisma.staffProfile.findMany({
      ...
      where: { id: { in: appointments.map((a) => a.doctorId) }, role: "DOCTOR" }
    });
    const withDoctors = appointments.map((a) => {
      const d = staffProfiles.find((s) => s.id === a.doctorId);
      return {
        ...a,
        doctor: {
          firstName: (d == null ? void 0 : d.firstName) || "",
          lastName: (d == null ? void 0 : d.lastName) || "",
          middleName: (d == null ? void 0 : d.middleName) || "",
          suffix: (d == null ? void 0 : d.suffix) || ""
        }
      };
    });
    return {
...
const getAppointment = protectedProcedure.input(getAppointmentSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const createAppointment = protectedProcedure.input(createAppointmentSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  ...
  try {
    if (!isWithinOfficeHours(time)) {
      return {
        success: false,
        message: "Appointment time must be within office hours (8:00 AM - 6:00 PM, excluding lunch 12:00 PM - 1:00 PM).",
        data: null
      };
    }
...
const updateAppointment = protectedProcedure.input(updateAppointmentSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const deleteAppointment = protectedProcedure.input(deleteAppointmentSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const assignRoom = protectedProcedure.input(assignRoomSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const updateAppointmentStatus = protectedProcedure.input(updateAppointmentStatusSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const getPatientAppointments = protectedProcedure.input(getPatientAppointmentsSchema).query(async ({ ctx, input }) => {
  ...
  try {
    ...
    const where = { patientId: currentPatient.id };
    if (doctorId) where.doctorId = doctorId;
    if (date) where.date = date;
    if (status) where.status = status;
    const total = await instancePrisma.appointment.count({ where });
...
const getPatientAppointment = protectedProcedure.input(getAppointmentSchema).query(async ({ ctx, input }) => {
...
const createPatientAppointment = protectedProcedure.input(createPatientAppointmentSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma, user } = ctx;
  ...
  try {
    ...
    const appointment = await instancePrisma.$transaction(async (tx) => {
...
const cancelPatientAppointment = protectedProcedure.input(updatePatientAppointmentSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma, user } = ctx;
  const { id } = input;
  try {
    const currentPatient = await getCurrentPatientOrNull(instancePrisma, user);
...
const getAvailableRooms = protectedProcedure.input(z.object({ timestamp: z.string().datetime(), excludeAppointmentId: z.string().uuid().optional() })).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  ...
  try {
    ...
    const occupied = existing.map((a) => a.facilityId).filter(Boolean);
    const available = clinicRooms.filter((r) => !occupied.includes(r.id));
...
const checkAvailability = protectedProcedure.input(checkAvailabilitySchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const getAvailableTimeSlots = protectedProcedure.input(getAvailableTimeSlotsSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  ...
  try {
    ...
    const booked = new Set(existing.map((a) => a.time));
...
const pharmacyBrandSchema = z.object({
  name: z.string().min(1, "Brand name is required.").max(255, "Brand name must be less than 255 characters.")
});
const getPharmacyBrandSchema = z.object({
  id: z.string().uuid("Invalid brand ID.")
});
const tablePharmacyBrandSchema = pharmacyBrandSchema.extend({
  id: z.string().uuid("Invalid brand ID."),
  createdAt: z.string().datetime(),
  updatedAt: z.string().datetime()
});
const createPharmacyBrandSchema = pharmacyBrandSchema;
const updatePharmacyBrandSchema = pharmacyBrandSchema.extend({
  id: z.string().uuid("Invalid brand ID.")
});
const deletePharmacyBrandSchema = z.object({
  id: z.string().uuid("Invalid brand ID.")
});

const pharmacyCategorySchema = z.object({
  name: z.string().min(1, "Category name is required.").max(255, "Category name must be less than 255 characters.")
});
const getPharmacyCategorySchema = z.object({
  id: z.string().uuid("Invalid category ID.")
});
const tablePharmacyCategorySchema = pharmacyCategorySchema.extend({
  id: z.string().uuid("Invalid category ID."),
  createdAt: z.string().datetime(),
  updatedAt: z.string().datetime()
});
const createPharmacyCategorySchema = pharmacyCategorySchema;
const updatePharmacyCategorySchema = pharmacyCategorySchema.extend({
  id: z.string().uuid("Invalid category ID.")
});
const deletePharmacyCategorySchema = z.object({
  id: z.string().uuid("Invalid category ID.")
});

const pharmacyItemFormSchema = z.enum([
  "TABLET",
  "CAPSULE",
  "SYRUP",
  "OINTMENT",
...
const pharmacyItemRouteSchema = z.enum([
  "INJECTION",
  "TOPICAL",
  "TRANSDERMAL",
  "OCULAR",
...
pharmacyItemFormSchema.options.map((option) => ({
  label: option,
  value: option
}));
pharmacyItemRouteSchema.options.map((option) => ({
  label: option,
  value: option
}));
const pharmacyItemSchema = z.object({
  ...
  stock: z.number().min(0, "Pharmacy item stock must be greater than 0.").max(1e6, "Pharmacy item stock must be less than 1000000."),
...
const getPharmacyItemSchema = z.object({
  id: z.string().uuid("Invalid pharmacy item ID.")
});
pharmacyItemSchema.extend({
  id: z.string().uuid("Invalid pharmacy item ID."),
...
const createPharmacyItemSchema = pharmacyItemSchema;
const updatePharmacyItemSchema = pharmacyItemSchema.extend({
  id: z.string().uuid("Invalid pharmacy item ID.")
});
const deletePharmacyItemSchema = z.object({
  id: z.string().uuid("Invalid pharmacy item ID.")
});

const pharmacySupplierSchema = z.object({
...
const getPharmacySupplierSchema = z.object({
  id: z.string().uuid("Invalid supplier ID.")
});
pharmacySupplierSchema.extend({
  id: z.string().uuid("Invalid supplier ID."),
  createdAt: z.string().datetime(),
  updatedAt: z.string().datetime()
}).omit({
...
const createPharmacySupplierSchema = pharmacySupplierSchema;
const updatePharmacySupplierSchema = pharmacySupplierSchema.extend({
  id: z.string().uuid("Invalid supplier ID.")
});
const deletePharmacySupplierSchema = z.object({
  id: z.string().uuid("Invalid supplier ID.")
});
...
const getPharmacyBrand = protectedProcedure.input(getPharmacyBrandSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const createPharmacyBrand = protectedProcedure.input(createPharmacyBrandSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { name } = input;
  try {
...
const updatePharmacyBrand = protectedProcedure.input(updatePharmacyBrandSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id, name } = input;
  try {
...
const deletePharmacyBrand = protectedProcedure.input(deletePharmacyBrandSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const pharmacyBrandsRouter = createTRPCRouter({
  getPharmacyBrands,
  getPharmacyBrand,
  createPharmacyBrand,
  updatePharmacyBrand,
...
const getPharmacyCategory = protectedProcedure.input(getPharmacyCategorySchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const createPharmacyCategory = protectedProcedure.input(createPharmacyCategorySchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { name } = input;
  try {
...
const updatePharmacyCategory = protectedProcedure.input(updatePharmacyCategorySchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id, name } = input;
  try {
...
const deletePharmacyCategory = protectedProcedure.input(deletePharmacyCategorySchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const pharmacyCategoriesRouter = createTRPCRouter({
...
const getPharmacyItem = protectedProcedure.input(getPharmacyItemSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  try {
...
const createPharmacyItem = protectedProcedure.input(createPharmacyItemSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const updatePharmacyItem = protectedProcedure.input(updatePharmacyItemSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const deletePharmacyItem = protectedProcedure.input(deletePharmacyItemSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const pharmacyItemsRouter = createTRPCRouter({
  getPharmacyItems,
  getPharmacyItem,
  createPharmacyItem,
  updatePharmacyItem,
...
const getPharmacySupplier = protectedProcedure.input(getPharmacySupplierSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const createPharmacySupplier = protectedProcedure.input(createPharmacySupplierSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const updatePharmacySupplier = protectedProcedure.input(updatePharmacySupplierSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const deletePharmacySupplier = protectedProcedure.input(deletePharmacySupplierSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const pharmacySuppliersRouter = createTRPCRouter({
...
function formatEnumLabel(value) {
  ...
  return normalized.split("_").filter(Boolean).map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(" ");
}

const inpatientTriageSchema = z.enum([
  "HIGH_PRIORITY",
  "MEDIUM_PRIORITY",
  "LOW_PRIORITY",
  "NON_URGENT"
]);
inpatientTriageSchema.options.map(
  (triage) => ({
    label: formatEnumLabel(triage),
    value: triage
  })
);
const inpatientDispositionSchema = z.enum([
  "ADMITTED",
  "DISCHARGED",
  "DISCONTINUED",
  "TRANSFERRED",
...
const inpatientEncounterBaseSchema = z.object({
  ...
  dispositionNote: z.string().min(1, "Disposition note is required.").optional().nullable()
});
const inpatientEncounterSchema = inpatientEncounterBaseSchema;
const getInpatientEncounterSchema = z.object({
  id: z.string().uuid("Invalid encounter ID.")
});
const getInpatientEncountersSchema = z.object({
  patientProfileId: z.string().uuid("Invalid patient profile ID.")
});
inpatientEncounterSchema.extend({
  id: z.string().uuid("Invalid encounter ID."),
...
const createInpatientEncounterSchema = inpatientEncounterBaseSchema.extend({
  patientProfileId: z.string().uuid("Invalid patient profile ID.")
});
const updateInpatientEncounterSchema = inpatientEncounterBaseSchema.extend({
  id: z.string().uuid("Invalid encounter ID."),
  patientProfileId: z.string().uuid("Invalid patient profile ID.")
});
const deleteInpatientEncounterSchema = z.object({
  id: z.string().uuid("Invalid encounter ID.")
});

const outpatientEncounterTypeSchema = z.enum([
  "CONSULTATION",
  "FOLLOW_UP",
  "LABORATORY",
  "RADIOLOGY",
...
const outpatientEncounterSchema = outpatientEncounterBaseSchema;
const getOutpatientEncounterSchema = z.object({
  id: z.string().uuid("Invalid encounter ID.")
});
const getOutpatientEncountersSchema = z.object({
  patientProfileId: z.string().uuid("Invalid patient profile ID.")
});
outpatientEncounterSchema.extend({
  id: z.string().uuid("Invalid encounter ID."),
  patientProfile: tablePatientProfileSchema,
  createdAt: z.string().datetime(),
  updatedAt: z.string().datetime()
...
const createOutpatientEncounterSchema = outpatientEncounterBaseSchema.extend({
  patientProfileId: z.string().uuid("Invalid patient profile ID.")
});
const updateOutpatientEncounterSchema = outpatientEncounterBaseSchema.extend({
  id: z.string().uuid("Invalid encounter ID."),
  patientProfileId: z.string().uuid("Invalid patient profile ID.")
});
const deleteOutpatientEncounterSchema = z.object({
  id: z.string().uuid("Invalid encounter ID.")
});

const getInpatientEncounters$1 = protectedProcedure.input(getInpatientEncountersSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { patientProfileId } = input;
  try {
    const inpatientEncounters = await instancePrisma.inpatientEncounter.findMany({
...
const getInpatientEncounter = protectedProcedure.input(getInpatientEncounterSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
    const inpatientEncounter = await instancePrisma.inpatientEncounter.findUnique({
      where: { id }
    });
    return {
      success: true,
...
const createInpatientEncounter = protectedProcedure.input(createInpatientEncounterSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const {
    patientProfileId,
    date,
...
const updateInpatientEncounter = protectedProcedure.input(updateInpatientEncounterSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const {
    id,
    date,
...
const deleteInpatientEncounter = protectedProcedure.input(deleteInpatientEncounterSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const inpatientEncountersRouter = createTRPCRouter({
  getInpatientEncounters: getInpatientEncounters$1,
...
const getOutpatientEncounters = protectedProcedure.input(getOutpatientEncountersSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { patientProfileId } = input;
  try {
    const outpatientEncounters = await instancePrisma.outpatientEncounter.findMany({
      where: { patientProfileId },
      include: {
        patientProfile: true
      }
...
const getOutpatientEncounter = protectedProcedure.input(getOutpatientEncounterSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
    const outpatientEncounter = await instancePrisma.outpatientEncounter.findUnique({
      where: { id }
    });
    return {
      success: true,
...
const createOutpatientEncounter = protectedProcedure.input(createOutpatientEncounterSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const {
    patientProfileId,
    date,
  ...
  try {
    const outpatientEncounter = await instancePrisma.outpatientEncounter.create({
      data: {
        patientProfileId,
        date,
        time,
...
const updateOutpatientEncounter = protectedProcedure.input(
  ...
  try {
    const outpatientEncounter = await instancePrisma.outpatientEncounter.update({
      where: { id },
...
const deleteOutpatientEncounter = protectedProcedure.input(deleteOutpatientEncounterSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const outpatientEncountersRouter = createTRPCRouter({
...
const encountersRouter = createTRPCRouter({
...
const minioClient = new Client({
  ...
  port: process.env.MINIO_PORT ? parseInt(process.env.MINIO_PORT) : void 0,
...
const uploadFile$1 = async (bucketName, file) => {
  try {
...
const MAX_FILE_SIZE$1 = 25 * 1024 * 1024;
const ALLOWED_FILE_TYPES$1 = [
  "image/jpeg",
  "image/png",
  "image/jpg"
];
const uploadFile = protectedProcedure.mutation(async ({ ctx }) => {
  try {
    const formData = await readMultipartFormData(ctx.event);
    ...
    if (file.data.length > MAX_FILE_SIZE$1) {
      return {
        success: false,
    ...
    if (!file.type || !ALLOWED_FILE_TYPES$1.includes(file.type)) {
      return {
        success: false,
    ...
    const { success, message, data } = await uploadFile$1("files", {
      data: file.data,
      name: fileName,
      type: file.type,
...
const getInsuranceProviderSchema = z.object({
  id: z.string().uuid("Invalid provider ID.")
});
const tableInsuranceProviderSchema = insuranceProviderSchema.extend({
  id: z.string().uuid("Invalid provider ID."),
  _count: z.object({
    claims: z.number()
  }).optional(),
...
const createInsuranceProviderSchema = insuranceProviderSchema;
const updateInsuranceProviderSchema = insuranceProviderSchema.extend({
  id: z.string().uuid("Invalid provider ID.")
});
const deleteInsuranceProviderSchema = z.object({
  id: z.string().uuid("Invalid provider ID.")
});

const insuranceClaimStatusSchema = z.enum([
  "PENDING",
  "APPROVED",
  "REJECTED",
  "PROCESSING",
...
insuranceClaimStatusSchema.options.map((option) => ({
...
const insuranceClaimSchema = z.object({
  ...
  message: z.string().max(1e3, "Message must be less than 1000 characters.").optional().nullable(),
  amount: z.number().min(0, "Amount must be greater than or equal to 0.").max(1e9, "Amount must be less than 1 billion.")
});
const getInsuranceClaimSchema = z.object({
  id: z.string().uuid("Invalid claim ID.")
});
insuranceClaimSchema.extend({
  id: z.string().uuid("Invalid claim ID."),
...
const createInsuranceClaimSchema = insuranceClaimSchema;
const updateInsuranceClaimSchema = insuranceClaimSchema.extend({
  id: z.string().uuid("Invalid claim ID.")
});
const deleteInsuranceClaimSchema = z.object({
  id: z.string().uuid("Invalid claim ID.")
});

const insuranceClaimItemSchema = z.object({
...
const getInsuranceClaimItemSchema = z.object({
  id: z.string().uuid("Invalid claim item ID.")
});
insuranceClaimItemSchema.extend({
  id: z.string().uuid("Invalid claim item ID."),
  claim: z.object({
    id: z.string().uuid(),
    status: z.string(),
...
const createInsuranceClaimItemSchema = insuranceClaimItemSchema;
const updateInsuranceClaimItemSchema = insuranceClaimItemSchema.extend({
  id: z.string().uuid("Invalid claim item ID.")
});
const deleteInsuranceClaimItemSchema = z.object({
  id: z.string().uuid("Invalid claim item ID.")
});
...
const getInsuranceProvider = protectedProcedure.input(getInsuranceProviderSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const createInsuranceProvider = protectedProcedure.input(createInsuranceProviderSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const updateInsuranceProvider = protectedProcedure.input(updateInsuranceProviderSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const deleteInsuranceProvider = protectedProcedure.input(deleteInsuranceProviderSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const insuranceProvidersRouter = createTRPCRouter({
...
const getInsuranceClaim = protectedProcedure.input(getInsuranceClaimSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const createInsuranceClaim = protectedProcedure.input(createInsuranceClaimSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const updateInsuranceClaim = protectedProcedure.input(updateInsuranceClaimSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const deleteInsuranceClaim = protectedProcedure.input(deleteInsuranceClaimSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const insuranceClaimsRouter = createTRPCRouter({
  getInsuranceClaims,
  getInsuranceClaim,
...
const getInsuranceClaimItem = protectedProcedure.input(getInsuranceClaimItemSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const createInsuranceClaimItem = protectedProcedure.input(createInsuranceClaimItemSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const updateInsuranceClaimItem = protectedProcedure.input(updateInsuranceClaimItemSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const deleteInsuranceClaimItem = protectedProcedure.input(deleteInsuranceClaimItemSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const insuranceClaimItemsRouter = createTRPCRouter({
...
const insuranceRouter = createTRPCRouter({
...
transactionStatusSchema.options.map((option) => ({
  label: option.charAt(0).toUpperCase() + option.slice(1).toLowerCase(),
  value: option
}));
const transactionStatus = z.string().pipe(transactionStatusSchema);
...
const getTransactionItemSchema = z.object({
...
const createTransactionItemSchema = transactionItemSchema;
const updateTransactionItemSchema = transactionItemSchema.extend({
  id: z.string().uuid("Invalid transaction item ID.")
});
const deleteTransactionItemSchema = z.object({
  id: z.string().uuid("Invalid transaction item ID.")
});
...
const createPaymentSchema = paymentSchema;
const updatePaymentSchema = paymentSchema.extend({
  id: z.string().uuid("Invalid payment ID.")
});
...
const paymentPlanCycleSchema = z.enum([
  "WEEKLY",
  "MONTHLY",
  "QUARTERLY",
  "YEARLY"
]);
paymentPlanCycleSchema.options.map((option) => ({
  label: option.charAt(0).toUpperCase() + option.slice(1).toLowerCase(),
  value: option
}));
...
const getPaymentPlanSchema = z.object({
...
const createPaymentPlanSchema = paymentPlanSchema;
const updatePaymentPlanSchema = paymentPlanSchema.extend({
  id: z.string().uuid("Invalid payment plan ID.")
});
const deletePaymentPlanSchema = z.object({
  id: z.string().uuid("Invalid payment plan ID.")
});

const paymentInstalmentSchema = z.object({
  ...
  amountDue: z.number().min(0, "Amount due must be greater than or equal to 0.").max(1e9, "Amount due must be less than 1 billion."),
  amountPaid: z.number().min(0, "Amount paid must be greater than or equal to 0.").max(1e9, "Amount paid must be less than 1 billion.")
});
const getPaymentInstalmentSchema = z.object({
  id: z.string().uuid("Invalid payment instalment ID.")
});
paymentInstalmentSchema.extend({
  id: z.string().uuid("Invalid payment instalment ID."),
  payment: z.object({
    id: z.string().uuid(),
    amount: z.number(),
...
const createPaymentInstalmentSchema = paymentInstalmentSchema;
const updatePaymentInstalmentSchema = paymentInstalmentSchema.extend({
  id: z.string().uuid("Invalid payment instalment ID.")
});
const deletePaymentInstalmentSchema = z.object({
  id: z.string().uuid("Invalid payment instalment ID.")
});
...
const getTransaction = protectedProcedure.input(getTransactionSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const createTransaction = protectedProcedure.input(createTransactionSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const updateTransaction = protectedProcedure.input(updateTransactionSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const deleteTransaction = protectedProcedure.input(deleteTransactionSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const getTransactionItem = protectedProcedure.input(getTransactionItemSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const createTransactionItem = protectedProcedure.input(createTransactionItemSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const updateTransactionItem = protectedProcedure.input(updateTransactionItemSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const deleteTransactionItem = protectedProcedure.input(deleteTransactionItemSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const transactionItemsRouter = createTRPCRouter({
  getTransactionItems,
  getTransactionItem,
...
const getPayment = protectedProcedure.input(getPaymentSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const createPayment = protectedProcedure.input(createPaymentSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const updatePayment = protectedProcedure.input(updatePaymentSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const deletePayment = protectedProcedure.input(deletePaymentSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const getPaymentPlan = protectedProcedure.input(getPaymentPlanSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const createPaymentPlan = protectedProcedure.input(createPaymentPlanSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const updatePaymentPlan = protectedProcedure.input(updatePaymentPlanSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const deletePaymentPlan = protectedProcedure.input(deletePaymentPlanSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const paymentPlansRouter = createTRPCRouter({
  getPaymentPlans,
  getPaymentPlan,
  createPaymentPlan,
  updatePaymentPlan,
...
const getPaymentInstalment = protectedProcedure.input(getPaymentInstalmentSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const createPaymentInstalment = protectedProcedure.input(createPaymentInstalmentSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const updatePaymentInstalment = protectedProcedure.input(updatePaymentInstalmentSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const deletePaymentInstalment = protectedProcedure.input(deletePaymentInstalmentSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const paymentInstalmentsRouter = createTRPCRouter({
...
const getFacilityLogs = protectedProcedure.input(getLogsSchema.optional()).query(async ({ ctx }) => {
  const { instancePrisma } = ctx;
  try {
...
const addFacilityLog = protectedProcedure.input(addLogSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  try {
...
const facilityLogRouter = createTRPCRouter({
  getFacilityLogs,
  addFacilityLog
});

const getLogs = protectedProcedure.query(async ({ ctx }) => {
  const { instancePrisma } = ctx;
  try {
...
const medicalRecordStatusSchema = z.enum(["PENDING", "PROCESSING", "COMPLETED", "CANCELLED"]);
medicalRecordStatusSchema.options.map((status) => ({
  label: status.charAt(0) + status.slice(1).toLowerCase(),
  value: status
}));
const medicalRecordRequestSchema = z.object({
...
const getMedicalRecordRequestSchema = z.object({
  id: z.string().uuid("Invalid medical record request ID.")
});
medicalRecordRequestSchema.extend({
  id: z.string().uuid("Invalid medical record request ID."),
...
const createMedicalRecordRequestSchema = medicalRecordRequestSchema.omit({ fileUrl: true, status: true });
const updateMedicalRecordRequestSchema = medicalRecordRequestSchema.extend({
  id: z.string().uuid("Invalid medical record request ID.")
});
const deleteMedicalRecordRequestSchema = z.object({
  id: z.string().uuid("Invalid medical record request ID.")
});
z.object({
  id: z.string().uuid(),
...
const getMedicalRecordRequests = protectedProcedure.input(medicalRecordStatusSchema.optional()).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const status = input != null ? input : void 0;
  try {
...
const getMedicalRecordRequest = protectedProcedure.input(getMedicalRecordRequestSchema).query(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const createMedicalRecordRequest = protectedProcedure.input(createMedicalRecordRequestSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  ...
  try {
    const request = await instancePrisma.medicalRecordRequest.create({
      data: {
        ...
        type,
        status: "PENDING",
        // Always set to PENDING for new requests
        fileUrl: ""
        // Will be updated later when file is uploaded
      },
      include: {
        patientProfile: true,
        inpatientEncounter: true
...
const updateMedicalRecordRequest = protectedProcedure.input(updateMedicalRecordRequestSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
...
const deleteMedicalRecordRequest = protectedProcedure.input(deleteMedicalRecordRequestSchema).mutation(async ({ ctx, input }) => {
  const { instancePrisma } = ctx;
  const { id } = input;
  try {
...
const getPatientProfiles = protectedProcedure.query(async ({ ctx }) => {
  const { instancePrisma } = ctx;
  try {
...
const getInpatientEncounters = protectedProcedure.query(async ({ ctx }) => {
  const { instancePrisma } = ctx;
  try {
...
const medicalRecordRequestsRouter = createTRPCRouter({
...
const medicalRecordsRouter = createTRPCRouter({
...
const appRouter = createTRPCRouter({
  ...
  facilityLogs: facilityLogRouter,
  // Add facility log router here
  logs: logsRouter,
  medicalRecords: medicalRecordsRouter
  // Add medical records router here
});
